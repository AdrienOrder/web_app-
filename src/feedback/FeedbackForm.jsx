...
// Импортируем хук useForm из библиотеки react-hook-form для упрощения работы с формами 
// (включает обработку валидации, ошибок и состояния).
...
// Импортируем контекст для работы с темой (ThemeContext), чтобы можно было менять визуальное оформление 
// в зависимости от темы (темная или светлая).
...
const FeedbackForm = () => {
    ...
    // Извлекаем роль текущего пользователя из контекста аутентификации (AuthContext), чтобы ограничить доступ к форме 
    // для определенных пользователей.

    const onSubmit = useCallback((data) => {
        // Функция onSubmit, которая будет вызвана при отправке формы. Используем useCallback, чтобы не пересоздавать её 
        // на каждом рендере.
